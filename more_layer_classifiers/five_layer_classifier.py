...
#     save_weights = True  # Tells whether to save weights... currently not used
    ...
    batch_size = 7000   # the training set is broken up into batches, 
    ...
    model_name = "fiveLayerModel_" + args.channel + "_corrCut_" + str(correlation_cut)  + "_weights_" + weight_cmd + "_drop_" + str(drop)
    ...
    model_name += "batch_" + str(batch_size) # was 5000 before adding this
    ...
    if correlation_cut > 0:
        data = ProductionModeDataset(file, correlation_cut=correlation_cut, cut_version=args.cut_version, remove = False, include_qg = qg_cmd)
    else:
        data = ProductionModeDataset(file, include_qg = qg_cmd, remove = False)
...
if opt.correlation_cut > 0:
    os.makedirs("../data/more_layers/", exist_ok=True)
...
np.save("../data/more_layers/train_val_datasets/training_dataset_" + opt.model_name + ".npy", opt.data.get_training_data())
np.save("../data/more_layers/train_val_datasets/validation_dataset_" + opt.model_name + ".npy", opt.data.get_eval_data())
...
class Classifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(64, opt.output_num),   # last layer
            nn.LeakyReLU(0.2, inplace=True))
...
if opt.load_model:
    classifier.load_state_dict(torch.load("../models/more_layers/" + opt.model_name + ".pt")) #load module with same name
...
if opt.load_model:
    optimizer.load_state_dict(torch.load("../models/more_layers/optimizer_" + opt.model_name + ".pt"))
...
for epoch in range(opt.n_epochs):   # Loop through all epochs
    ...
    if epoch % 10 == 0:
        ...
        if small_loss > loss_val:   # compare to see if the loss has decreased
            ...
            torch.save(classifier.state_dict(), "../models/more_layers/" + opt.model_name + ".pt")   # save the new (and better) model
            torch.save(optimizer.state_dict(), "../models/more_layers/optimizer_" + opt.model_name + ".pt") # save the optimizer state
            
        if epoch % 50 == 0:
            # save the loss arrays
            np.save("../data/more_layers/" + opt.model_name +  "_loss_val_array.npy",loss_val_array)
            np.save("../data/more_layers/" + opt.model_name + "_loss_train_array.npy",loss_array)
            if opt.draw:
                #----------------------------------
