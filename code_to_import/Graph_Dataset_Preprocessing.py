...
class GraphProductionModeDataset(Dataset):
    ...
    def __init__(self, root, gen=True, split=True, normalize=True, remove=True, train=True, correlation_cut=-1.0, cut_version=-1, 
        ...
        if correlation_cut > 0:
            ...
            if cut_version > 0:
                to_remove = np.load("../analysis_code/results/inputs_to_remove_cut_" + str(correlation_cut)                                    + "v" + str(cut_version) + ".npy")
        ...
        if remove:
            ...
            if include_qg:
                ...
                """
                self.events_array = np.delete(self.events_array, list(range(num_qqbar, first)) +                                              list(range(last, max_len)), 0)
        ...
        if not remove:
            ...
            #NOTE: THIS ONLY WORKS WITHOUT ANY QG OR GQ EVENTS!!!!!!!!!
            num_gg = len(self.events_array[:,index]) - num_qqbar
            print("num gg is " + str(num_gg))
            #########################################################################
#             for loop in range(num_gg - num_qqbar):
